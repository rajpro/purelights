...
class Marketing extends CI_Controller {
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('mrk_full_name', 'Full Name', 'required|alpha_numeric_spaces');
		$this->form_validation->set_rules('mrk_address', 'Address', 'required');
		...
		$this->form_validation->set_rules('mrk_account_no', 'Account Number', 'required|numeric|min_length[10]|max_length[20]');
		...
		if($this->form_validation->run() == TRUE){
			...
			$post['mrk_address'] = json_encode(['mrk_address'=>$post['mrk_address'], 'state'=>$post['state'], 'district'=>$post['district']]);
			$post['mrk_bank_detail'] = json_encode(array_merge($this->_get_ifsc($post['ifsc']), ['ACCOUNT'=>$post['mrk_account_no']]));
			$post['mrk_referral'] = $this->MarketingDB->referral();
			$post['mrk_referred_by'] = $this->MarketingDB->referred_by();
			$post['mrk_level'] = $this->MarketingDB->level();
			$post['mrk_status'] = 'deactive';
			
			// username password
			$user['username'] = $post['mrk_referral'];
			$user['password'] = password_hash($post['password'], PASSWORD_BCRYPT, ['cost'=>12]);
			$user['usertype'] = 'marketing';
			$user['email'] = $post['mrk_referral']."@purelights.in";
			$user['status'] = 'deactive';

			unset($post['state'], $post['district'], $post['ifsc'], $post['mrk_account_no'], $post['password']);
	...
	public function edit($id)
	{
		...
		$data['data']['model'] = $this->MarketingDB->findRow(['mrk_id'=>$id]);
